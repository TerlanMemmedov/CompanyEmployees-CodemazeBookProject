...
{
    public class EmployeeLinks : IEmployeeLinks //HttpContext error verir Interface-de (tanimir)
    {
        ...
        private List<Link> CreateLinksForEmployee(HttpContext httpContext, Guid companyId, Guid id, string fields = "")
        {
            ...
            {
                new Link(_linkGenerator.GetUriByAction(httpContext, "GetEmployee", "Employees",
                    values: new { companyId, id, fields}),
                    "self",
                    "GET"),
                new Link(_linkGenerator.GetUriByAction(httpContext, "DeleteEmployee","Employees",
                    values: new { companyId, id }),
                    "delete_employee",
                    "DELETE"),
                new Link(_linkGenerator.GetUriByAction(httpContext, "UpdateEmployeeForCompany","Employees",
                    values: new { companyId, id }),
                    "update_employee",
                    "PUT"),
                new Link(_linkGenerator.GetUriByAction(httpContext, "PartiallyUpdateEmployeeForCompany","Employees",
                    values: new { companyId, id }),
        ...
        {
            employeesWrapper.Links.Add(new
                Link(_linkGenerator.GetUriByAction(httpContext, "GetEmployeesForCompany", values: new { }),
                "self",
                "GET"));
